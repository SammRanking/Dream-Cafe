...
exports.run = async (client, message, args) => {
    ...
    message.channel.send({embed: {
        color: 9240450,
        description: `**Success!** Ranked ${username} to ${setRankResponse.name}`,
        author: {
            name: message.author.tag,
            icon_url: message.author.displayAvatarURL()
        }
    ...
    logchannel.send({embed: {
        color: 2127726,
        description: `<@${message.author.id}> has ranked ${username} from ${rankNameInGroup} to ${setRankResponse.name}.`,
        author: {
            name: message.author.tag,
            icon_url: message.author.displayAvatarURL()
        },
