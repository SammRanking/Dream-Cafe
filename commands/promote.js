...
exports.run = async (client, message, args) => {
    ...
    try {
        ...
        return message.channel.send({embed: {
            color: 16733013,
            description: `{username} is not a Roblox user.`,
            author: {
                name: message.author.tag,
                icon_url: message.author.displayAvatarURL
            }
    ...
    message.channel.send({embed: {
        color: 9240450,
        description: `**Success!** Promoted ${username} to ${promoteResponse.newRole.name}`,
        author: {
            name: message.author.tag,
            icon_url: message.author.displayAvatarURL()
        }
    ...
    logchannel.send({embed: {
        color: 2127726,
        description: `<@${message.author.id}> has promoted ${username} from ${rankNameInGroup} to ${promoteResponse.newRole.name}.`,
        author: {
            name: message.author.tag,
            icon_url: message.author.displayAvatarURL()
        },
