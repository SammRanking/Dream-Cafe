...
exports.run = async (client, message, args) => {
    ...
    message.channel.send({embed: {
        color: 9240450,
        description: `**Success!** Demoted ${username} to ${demoteResponse.newRole.name}`,
        author: {
            name: message.author.tag,
            icon_url: message.author.displayAvatarURL()
        }
    ...
    logchannel.send({embed: {
        color: 2127726,
        description: `<@${message.author.id}> has demoted ${username} from ${rankNameInGroup} to ${demoteResponse.newRole.name}.`,
        author: {
            name: message.author.tag,
            icon_url: message.author.displayAvatarURL()
        },
